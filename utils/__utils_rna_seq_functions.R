...
myload_filter_ribo <- function(rdata, filter_ribo = FALSE, ribo_file = NULL) {
  load(rdata)  # loads the 'dds' object
  if (filter_ribo) {
    if (is.null(ribo_file) || !file.exists(ribo_file)) {
      stop("ribosomal gene file not provided or not found.")
    }
  # Read ribosomal genes as a vector (no header, one gene per line)
  ribo_genes <- readLines(ribo_file)
  ribo_genes <- ribo_genes[nzchar(trimws(ribo_genes)) & !grepl("^#", ribo_genes)]
  # Build a pattern for partial matching
  pattern <- paste(ribo_genes, collapse = "|")
  # Keep only rows that do NOT match any ribosomal gene
  keep_rows <- !grepl(pattern, rownames(dds))
  dds <- dds[keep_rows, ]
  }
  return(dds)
}
...
gen_comparison <- function(samples_df, e_con, e_time, c_con, c_time,
                           exclude = NULL,
                           comp_id,
                           comp_descr = NULL,
                           e_descr = NULL,
  ...
  comp_id = comp_id
  comp_descr = paste(exp_d, "vs", contr_d)
  e_descr = paste(exp_d, "(exp)")
  c_descr = paste(contr_d, "(ctrl)")
  
  # Gruppo experimental (e)
  e_samples <- samples_df %>%
    filter(condition %in% e_con) %>%
    filter(time %in% e_time)
  
  # Gruppo control (c)
  c_samples <- samples_df %>%
    filter(condition %in% c_con) %>%
    filter(time %in% c_time)
  
  # Costruzione tabella confronto
  comp_df <- bind_rows(
  ...
  # Escludi campioni se specificato
  if (!is.null(exclude)) {
    comp_df <- comp_df %>% filter(!(sample %in% exclude))
  }
  
  # Ritorna anche metadati
  return(list(
...
mydeseq2 = function(counts, min_reads, gene_name_type, sample_info, min_sample = 5, design_formula = ~ batch + condition, contrast_variable, test = "Wald", ...) {
  ...
  sample_info[[contrast_variable]] <- make.names(as.character(sample_info[[contrast_variable]]))
  sample_info[[contrast_variable]] <- as.factor(sample_info[[contrast_variable]])
  ...
  for (i in 1:(length(conds) - 1)) {
    for (j in (i + 1):length(conds)) {
      ...
      res$comparison_numerator_vs_denominator <- paste0(conds[i], "_vs_", conds[j])
...
specific_deseq2 = function(counts, min_reads, sample_info, exp, contr, min_sample = 5, design_formula = ~ batch + condition, contrast_variable, gene_name_type = "ENSEMBL", test = "Wald", ...) {
  ...
  # Align sample_info rows with count matrix columns
  sample_info <- sample_info[colnames(counts), ]
  # Make contrast_variable names syntactically valid
  sample_info$contrast_variable <- make.names(sample_info[[contrast_variable]])
  
  # create the correct control
  sample_info$contrast_variable = as.factor(sample_info$contrast_variable)
  sample_info$contrast_variable <- relevel(sample_info$contrast_variable, ref = contr)
  ...
  contrast <- c(contrast_variable, exp, contr)
  res <- results(diff, contrast = contrast, ...)
  ...
  rld_subset <- rld[, coldata$contrast_variable %in% c(exp, contr)]
  coldata_subset <- coldata[coldata$contrast_variable %in% c(exp, contr), ]
...
preprocessing_PCA = function(counts, min_reads, sample_info, normalization, min_sample = 5, design_formula = ~ batch + condition, contrast_variable, ...) {
  # Filter lowly expressed genes
  counts = counts[rowSums(counts) > min_reads & rowSums(counts > 1) > min_sample, ]
  # Align sample_info rows with count matrix columns
  sample_info <- sample_info[colnames(counts), ]
...
my_genetable = function(res, title_1vs2, qvalue, logfc, path_option = '', ...) {
  gene_symbols <- res$symbol
  gene_df <- bitr(gene_symbols, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)
  # not_mapped <- setdiff(gene_symbols, gene_df$SYMBOL)
  # cat("Unmapped Gene symbols (", length(not_mapped), "):\n", paste(not_mapped, collapse = ", "), "\n")
  
  # Aggiungi ENTREZID a 'res'
  res$SYMBOL <- res$symbol
  res <- left_join(as.data.frame(res), gene_df, by = "SYMBOL")
  ...
  go_annot <- suppressMessages(AnnotationDbi::select(org.Hs.eg.db,
                                                     keys = res$ENTREZID,
                                                     keytype = "ENTREZID",
                                                     columns = c("GO", "ONTOLOGY")))
  
  # Separate GO annotations by ontology
  go_bp <- go_annot %>%
    filter(ONTOLOGY == "BP") %>%
    group_by(ENTREZID) %>%
  ...
  msig_hallmark_by_gene <- msigdb_hallmark %>%
    filter(entrez_gene %in% res$ENTREZID) %>%
    group_by(entrez_gene) %>%
    summarise(HALLMARK_Pathways = paste(unique(gs_name), collapse = "; "))
  colnames(msig_hallmark_by_gene)[1] <- "ENTREZID"
  
  res <- left_join(res, go_bp, by = "ENTREZID")
  res <- left_join(res, go_mf, by = "ENTREZID")
  res <- left_join(res, go_cc, by = "ENTREZID")
  res <- left_join(res, msig_hallmark_by_gene, by = "ENTREZID")
  
  res <- res %>%
    dplyr::select(SYMBOL, ENTREZID, log2FoldChange, padj, FoldChange, 
           differentially_expressed, GO_BP, GO_MF, GO_CC, HALLMARK_Pathways, everything())
  ...
  # write.table(x = deg, file = paste0(path_option, "./output/deg_", gsub("__", "_", gsub( " ", "_", title_1vs2)), ".tsv"), quote=F, sep ="\t")
...
my_heatmaps <- function(deg, rld, title_1vs2, max_genes_show = 50) {
  ...
  deg_filtered <- deg[deg$symbol %in% rownames(rld), ]
  top20_genes <- deg_filtered %>% as.data.frame() %>% arrange(padj) %>% head(20) %>% dplyr::pull(gene)
  # Ensure top 20 genes are present in the assay matrix
  top20_genes <- intersect(top20_genes, rownames(rld))
  ...
  all_de_genes <- intersect(all_de_genes, rownames(rld))
  ...
  if (length(all_de_genes) == 0) {
    ...
    if (nrow(expr_de_scaled) == 0) {
      ...
      show_gene_names <- nrow(expr_de_scaled) <= max_genes_show
  ...
  return(list(h_20 = h_20, h_all = h_all, expr_de_scaled = expr_de_scaled))
}
...
run_gsea_hallmark_local = function(genelist, gmt_file, pvalueCutoff = 0.2) {
  library(clusterProfiler)
  
  # Read GMT file locally
  hallmark_df <- read.gmt(gmt_file)
...
goenrichment_test = function(markers, down = FALSE, n_genes = 100, logfc_col = "avg_log2FC", 
                        pvalueCutoff = 0.05, qvalueCutoff = 0.05, gene_name_type = "ENSEMBL") {

  # Invert logFC if analyzing downregulated genes
  if (down) {
    markers[[logfc_col]] <- -markers[[logfc_col]]
  }
  # Keep only genes with positive (or inverted) logFC
  up <- markers[markers[[logfc_col]] > 0, ]
  if (nrow(up) == 0) {
    warning("⚠️ No genes passed the logFC filter.")
    return(NULL)
  }
  # Sort genes by descending logFC
  up <- up[order(up[[logfc_col]], decreasing = TRUE), ]
  # Select top genes
  marker_genes <- if (nrow(up) < n_genes) up$gene else head(up$gene, n_genes)
  if (length(marker_genes) == 0) {
    warning("⚠️ No marker genes available after selection.")
    return(NULL)
  }

  # Check that at least one marker gene is valid for the specified gene name type
  valid_keys <- keys(org.Hs.eg.db, keytype = gene_name_type)
  if (!any(marker_genes %in% valid_keys)) {
    warning("⚠️ None of the marker genes are valid keys for ", gene_name_type, ". Skipping enrichment.")
    return(NULL)
  }
  ...
  if (length(entrez_ids) == 0) {
    warning("⚠️ No valid ENTREZ IDs after mapping. Skipping enrichment.")
    return(NULL)
  }
  # Run GO enrichment
  go_enrichment <- enrichGO(
  ...
  # Filter by padj and qvalue
  go_enrichment@result <- go_enrichment@result[
    go_enrichment@result$p.adjust < pvalueCutoff &
    go_enrichment@result$qvalue < qvalueCutoff, 
  ]
  return(go_enrichment)
}
